...
| Acronyms / glossary entries | `./abbreviations.tex` — loaded by `glossaries` via `\loadglsentries{abbreviations}` in `main.tex` |
| Bibliography | `./zotero.bib` (auto-generated by Zotero) + `./law.bib` (manual entries) — OSCOLA via biblatex |
...
- Before suggesting LaTeX commands, check `main.tex` preamble for loaded packages — don't assume what's available.
...
- **Command:** `latexmk main.tex` from the repo root. `.latexmkrc` selects `xelatex` (required by `fontspec` + the Scala Pro mainfont) and writes artifacts to `Output/`.
- **Glossary + indexes:** `main.tex` loads `glossaries` (acronyms from `abbreviations.tex`) and `imakeidx` with two indexes — `cases` and `legislation` — wired into biblatex-oscola via `indexing=cite`. `.latexmkrc` adds custom dependencies so latexmk regenerates `.gls`/`.acr` and rebuilds the indexes automatically; a single `latexmk` run is enough.
- **Bibliography:** biblatex with `style=oscola`, `ibid=true`. `zotero.bib` is auto-generated by Zotero — do not hand-edit it. New manual legal entries (cases, treaties, regulations, directives, decisions) go in `law.bib`.
...
1. Read the current draft directly from `Chapters/chapter_*.tex` — **always re-read**, never rely on cached versions. Skim `main.tex` too: it's the source of truth for which packages and macros are available, so don't suggest a new package without checking the preamble first.
