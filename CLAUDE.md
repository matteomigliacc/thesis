...
- `.gitignore` excludes `.DS_Store`, `Proposal/`, `AI/`, `flowchart`, `Output/`, `.claude/`, and `*.code-workspace`.
...
- **Bibliography:** biblatex with `style=oscola`, `ibid=true`. `bibliography/zotero.bib`. New manual legal entries (cases, treaties, regulations, directives, decisions) go in `bibliography/law.bib`.
