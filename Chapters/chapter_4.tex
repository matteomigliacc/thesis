...
\todo[inline]{This chapter needs to be connected to the duty of care more explicitly. I think the elements are there, and I tried to make the connection in the end, but it should be addressed throughout more. Also needs more citing.}
...
The simple procedure can also be turned into a complex one at the request of the \glspl{sac} with seemingly no proof.
...
\todo[inline]{I should also talk about the fact that the EDPB would also see its ability to be more careful in decisions by having more evidence to rule on.}
...
Recalling the aspect of the duty of care that requires administrations to investigate matters to their fullest, the Procedural Regulation does provide a framework that, at the very least, allows the actors responsible to enforce the \gls{gdpr} to `peer pressure' each other into fully investigating violations.
That peer pressure however will only be exerted by fellow \glspl{sa} that suffer from chronic underfunding and a lack of resources.
The effectiveness of this mechanism will depend on the willingness of \glspl{ms} to fund their national \gls{dpa} adequately.

It should also be noted that nothing, under the previous system of enforcement, prevented \glspl{sa} from cooperating with each other.
The Procedural Regulation does not invent radical new tools for cooperation.
It merely forces \glspl{sa} to use the tools already present in a way that sets the scope of an investigation before the procedure starts.