...
\label{sec:ch5-duty-to-give-reasons-procedural-regulation}
...
As discussed in \cref{sec:ch2-duty-to-give-reasons}, administrations have a duty to provide the parties affected by their decisions (i) a concise but
...
\label{sec:ch5-pre-reform-reason-giving}
...
Notably, \glspl{sa} use voluntary mutual assistance to discuss and solve general legal questions relating to the application of the regulation, thus contributing to a uniform understanding of the \gls{gdpr} and helping each other in interpreting the legal framework.\autocite[4.1.2]{edpbContributionGDPREvaluation2023}

According to the \gls{edpb}'s interpretation of Article 60(3) \gls{gdpr}, the \gls{lsa} already had a duty to consider the \glspl{sac}' views all along the procedure, and not just when a draft decision was issued.\autocite[124--125]{edpbGuidelines022022}
This obligation stems from the duty of \glspl{sa} to reach consensus on an investigation from the earliest stages, thus creating a mutual exchange of views and legal assessments throughout the procedure, to reach a decision that is consensual.\autocite[125--128]{edpbGuidelines022022}
...
\glspl{rro}, as seen in \cref{sec:ch3-cooperation-procedure}, must be `reasoned'.
...
Looking at Craig's third function of reason-giving, these moments may function to produce more carefully reasoned decisions, as the \glspl{sa} are required to engage with their peers.
This mechanism fails however when the informational asymmetries between the \gls{lsa} and the \glspl{sac} do not allow the \glspl{sac} to meaningfully challenge the \gls{lsa}'s reasoning and form their own opinion,\autocite[504]{mustertProceduralisingEnforcement2026} as seen in \cref{sec:ch3-pre-reform-mechanism,sec:ch4-pre-reform-system}.

\subsection{Reason-Giving in the Post-Reform Mechanism}
\label{sec:ch5-post-reform-reason-giving}
\subsubsection{The Cooperation File and the Ability of the SAs to Formulate their Views}
\label{sec:ch5-cooperation-file}
...
\label{sec:ch5-new-procedural-steps}
...
In addition to the summary of key issues, the Procedural Regulation makes other explicit references to the \glspl{sa}' duty to reason with each other along the procedure.
...
The \gls{edpb} must now decide whether the \glspl{rro} are relevant and reasoned and communicate this to the \glspl{sac} within four weeks of referral.\autocite[27(5)]{ProceduralRegulation}

The institution of these new procedural steps may provide the integrated system of enforcement with more avenues to exchange views and test the logic of the legal assessments made.\autocite[1703]{liShoulderEnforcementEuropean2022}

\subsubsection{Early Resolution}
\label{sec:ch5-early-resolution}

The early resolution stage presents an involution compared to the previous system of cooperation.
As seen in \cref{sec:ch4-early-resolution}, the early resolution procedure allows the \gls{lsa} to decide to close a case with a decision that cannot be challenged by a \gls{rro}.\autocite[23(1)(c)]{ProceduralRegulation}
...
One advantage of resorting to procedures at the \gls{edpb} level more frequently is that the decisions of the \gls{edpb} are public.\autocite[39]{EDPBRulesofProcedure}
...
\label{sec:ch5-enforceability}

As seen in \cref{sec:ch4-enforceability}, Article 14 of the Procedural Regulation states that non-compliance with the time limits imposed by the regulation does not affect the validity of the procedural steps or the final decision they lead to.\autocite[14]{ProceduralRegulation}
...
The lack of enforceability of new procedural steps may reproduce the same issues of informational asymmetry present in the pre-reform system, hindering the ability of the \glspl{sac} to meaningfully participate in the decision-making process.

\subsection{The Capacity of DPAs to Formulate their Views}
\label{sec:ch5-capacity}

As seen in \cref{sec:ch4-deadlines}, \glspl{dpa} are often under-resourced and under-staffed,\autocite[4.4]{edpbContributionGDPREvaluation2023} which pushes them to create diverging prioritization strategies at the national level,\autocite[501]{mustertProceduralisingEnforcement2026} as well as to use informal channels for cooperation to exchange information in order to more flexibly manage their increasing workload.\autocites[466--467]{hofmannmustertScholtenHandbook2023}[4.1.2]{edpbContributionGDPREvaluation2023}

The difficulties in the ability of \glspl{sa} to participate actively in cases do not only stem from a lack of procedural moments that require them to exchange views.
As seen in \cref{sec:ch5-pre-reform-reason-giving}, there was already a duty to exchange views all along the cooperation procedure that could be inferred from the law, as interpreted by the \gls{edpb}.

If the structural issues of under-resourcing are not addressed, \glspl{sa} may still resort to using more informal channels of cooperation, which do not carry the same legal obligations and time limits created by the Procedural Regulation.\autocite[467]{hofmannmustertScholtenHandbook2023}
...
\label{sec:ch5-interim-conclusion}
